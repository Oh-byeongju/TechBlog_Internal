...
1. **프로젝트 소개**
    ...
    <img width="100%" alt="ERD" src="https://github.com/user-attachments/assets/3cc00bc4-dd96-4a04-a997-c10dc4eabe6f"/>
...
<img width="100%" alt="ERD" src="https://github.com/user-attachments/assets/19bd741a-e8a1-4788-96d5-a36a5dc29991"/>
...
| **계층** | **기술 스택** | **역할** |
| --- | --- | --- |
| 클라이언트 | Chrome, Edge, Mobile Web | 사용자 UI 접근 |
| 프론트엔드 | Node.js, TypeScript, Next.js | SSR/CSR 렌더링, API 연동 |
| 백엔드 | Java 11, Spring Boot, Tomcat | 비즈니스 로직 처리, 데이터 제공 |
| DB | PostgreSQL | 데이터 저장 및 조회 |
| 외부 API | OpenAI | AI 기능 호출 (예: GPT 응답) |

---

## 5. 사용자 요청 흐름

<img width="100%" alt="ERD" src="https://github.com/user-attachments/assets/7ce83aa4-32cd-45a6-87f0-85342663c919"/>

- 사용자의 모든 요청은 `Next.js 미들웨어`를 거칩니다.
- 미들웨어에서는 쿠키에 저장된 JWT 토큰(`AccessToken`)을 읽어, `Authorization` 헤더에 추가합니다.
- 이후, 백엔드(Spring Boot) API 서버로 **요청을 리다이렉트**하면서, 필요한 헤더 정보들을 함께 전달합니다.
